...
int main(void)
{
  ...
  while (1)
  {
	...
	// Convert ADC value to a string for display
	sprintf(adc_str, "ADC Value: %lu", adc_val);
	...
	// Convert ADC value to CCR value for PWM
	uint32_t CCR = ADCtoCCR(adc_val);

	__HAL_TIM_SetCompare(&htim3, TIM_CHANNEL_3, CCR);


	// Display ADC value on the LCD (optional)
	sprintf(adc_str, "ADC Value: %lu", adc_val);
	writeLCD(adc_str);
...
void EXTI0_1_IRQHandler(void)
{
	...
	// Check if the button press is a valid one (not due to noise or bouncing)
	    if (HAL_GetTick() - prev_millis >= 100) {
	        // Toggle LED7_Pin (PB7) state
	        HAL_GPIO_TogglePin(GPIOB, LED7_Pin);

	        // Update the previous millis for debouncing
...
void writeLCD(char *char_in){
    ...
    lcd_command(HOME);  // Move cursor to the home position

    // Write the input string to the LCD
